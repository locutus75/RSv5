...
export async function sendViaSMTP({ tenant, parsed, rcpts, envelopeFrom, smtpServer, localAddress }) {
  ...
  if (localAddress && localAddress !== "0.0.0.0") {
    ...
    transporterConfig.getSocket = function(options, callback) {
      ...
      // Op Windows: gebruik connect() met localAddress optie direct
      // Dit werkt beter dan eerst bind() en dan connect()
      socket.on('error', (err) => {
      ...
      // Gebruik connect() met localAddress optie - dit werkt beter op Windows
      socket.connect({
        port: options.port,
        host: options.host,
        localAddress: localAddress, // Expliciet binden aan dit IP adres
        localPort: 0 // Laat OS een vrije poort kiezen
      }, () => {
        ...
        if (localAddr !== localAddress) {
          ...
          console.warn(`   💡 Dit is een Windows netwerkstack beperking`);
        } else {
