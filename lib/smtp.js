...
export async function sendViaSMTP({ tenant, parsed, rcpts, envelopeFrom, smtpServer, localAddress }) {
  ...
  if (localAddress && localAddress !== "0.0.0.0") {
    ...
    transporterConfig.getSocket = function(options, callback) {
      ...
      // Gebruik een willekeurige vrije poort (0) en het specifieke IP adres
      try {
        socket.bind(0, localAddress, (bindErr) => {
          if (bindErr) {
            console.error(`   ❌ Fout bij binden aan ${localAddress}: ${bindErr.message} (code: ${bindErr.code})`);
            console.error(`   💡 Controleer of het IP adres ${localAddress} beschikbaar is op het systeem`);
            return callback(bindErr);
          }
          
          const boundAddress = socket.address();
          console.log(`   ✅ Socket gebonden aan lokaal adres: ${boundAddress.address}:${boundAddress.port} (family: ${boundAddress.family})`);
          
          // Verifieer dat de binding correct is
          if (boundAddress.address !== localAddress) {
            console.warn(`   ⚠️ WAARSCHUWING: Socket gebonden aan ${boundAddress.address} in plaats van geconfigureerd ${localAddress}`);
          }
          ...
          socket.connect(options.port, options.host, () => {
            ...
            if (localAddr !== localAddress) {
              ...
              console.warn(`   💡 Dit kan gebeuren als het IP adres niet beschikbaar is op de interface`);
              console.warn(`   💡 Mogelijk gebruikt het OS het eerste IP adres van de interface`);
            } else {
