...
/**
 * Helper functie om Windows interface informatie op te halen voor een IP adres
 * @param {String} localAddress - Het IP adres waarvoor we interface info nodig hebben
 * @returns {Promise<Object>} Object met targetInterface en interfaceIndex
 */
async function getWindowsInterfaceInfo(localAddress) {
  const isWindows = os.platform() === 'win32';
  let targetInterface = null;
  let interfaceIndex = null;
  
  if (!isWindows) {
    return { targetInterface: null, interfaceIndex: null };
  }
  
  // Vind de interface die bij dit IP adres hoort
  const interfaces = os.networkInterfaces();
  for (const [name, addrs] of Object.entries(interfaces)) {
    if (addrs) {
      for (const addr of addrs) {
        if (addr.address === localAddress && addr.family === 'IPv4') {
          targetInterface = name;
          console.log(`   🔍 Interface gevonden voor ${localAddress}: ${name}`);
          break;
        }
      }
  ...
  // Probeer interface index te vinden via netsh
  if (targetInterface) {
    try {
      const netshResult = await new Promise((resolve) => {
        const proc = spawn('netsh', ['interface', 'ipv4', 'show', 'interfaces']);
        let output = '';
      ...
      // Parse interface index uit netsh output
      const lines = netshResult.output.split('\n');
      for (const line of lines) {
        if (line.includes(targetInterface) || targetInterface.includes(line.trim())) {
          const match = line.match(/^\s*(\d+)\s+/);
          if (match) {
            interfaceIndex = match[1];
            console.log(`   🔍 Interface index gevonden: ${interfaceIndex} voor ${targetInterface}`);
            break;
          }
        }
      }
    } catch (err) {
      console.log(`   ⚠️ Kon interface index niet bepalen: ${err.message}`);
    }
  }
  
  return { targetInterface, interfaceIndex };
}
...
export async function sendViaSMTP({ tenant, parsed, rcpts, envelopeFrom, smtpServer, localAddress }) {
  ...
  if (localAddress && localAddress !== "0.0.0.0") {
    ...
    transporterConfig.getSocket = function(options, callback) {
      ...
      // Op Windows: gebruik een andere aanpak voor socket binding
      // Windows heeft een bekende beperking waarbij het eerste IP adres van een interface wordt gebruikt
      // Probeer expliciete binding met socket opties
      const socket = new net.Socket({
        allowHalfOpen: false
      });
      
      // Stel socket opties in VOORDAT we binden
      socket.setNoDelay(true);
      socket.setKeepAlive(true, 0);
      
      // Error handler
      socket.on('error', (err) => {
        console.error(`   ❌ Socket error: ${err.message} (code: ${err.code})`);
        if (err.code === 'EADDRNOTAVAIL') {
          console.error(`   💡 Het IP adres ${localAddress} is mogelijk niet beschikbaar op deze interface`);
        } else if (err.code === 'EINVAL') {
          console.error(`   💡 Ongeldig IP adres of poort: ${localAddress}`);
        }
        callback(err);
      });
      
      // Probeer eerst te binden aan het specifieke IP adres
      // Op Windows moet dit gebeuren VOORDAT we connect() aanroepen
      try {
        // Bind aan het specifieke IP adres met poort 0 (laat OS poort kiezen)
        socket.bind(0, localAddress, () => {
          const boundAddr = socket.address();
          console.log(`   ✅ Socket gebonden aan: ${boundAddr.address}:${boundAddr.port} (family: ${boundAddr.family})`);
          
          // Verifieer binding
          if (boundAddr.address !== localAddress) {
            console.warn(`   ⚠️ Socket gebonden aan ${boundAddr.address} i.p.v. ${localAddress}`);
            console.warn(`   💡 Windows gebruikt mogelijk het eerste IP adres van de interface`);
          }
          
          // Nu verbinden - socket is al gebonden aan het juiste IP adres
          socket.connect(options.port, options.host, () => {
            ...
            if (localAddr !== localAddress) {
              console.warn(`   ⚠️ WAARSCHUWING: Socket gebruikt ${localAddr} i.p.v. ${localAddress}`);
              console.warn(`   💡 Windows netwerkstack gebruikt het eerste IP adres (${localAddr})`);
              console.warn(`   💡 Dit is een bekende Windows beperking bij meerdere IP's op één interface`);
              console.warn(`   💡 Oplossing: Configureer Windows routing of gebruik aparte interfaces`);
            } else {
        ...
        console.error(`   ❌ Exception bij binden: ${bindError.message}`);
        console.error(`   Stack: ${bindError.stack}`);
        return callback(bindError);
      }
...
export async function testSMTPConnection(smtpServer, localAddress) {
  ...
  if (localAddress && localAddress !== "0.0.0.0") {
    ...
    const isWindows = os.platform() === 'win32';
    const { targetInterface, interfaceIndex } = await getWindowsInterfaceInfo(localAddress);
    ...
    transporterConfig.getSocket = function(options, callback) {
      ...
      if (isWindows) {
        console.log(`   💡 Windows detecteerd - gebruik alternatieve aanpak`);
        console.log(`   💡 Windows heeft een bekende beperking waarbij socket.bind() het eerste IP adres gebruikt`);
        console.log(`   💡 Alternatieve oplossing: Configureer Windows routing tabel`);
        
        if (targetInterface && interfaceIndex) {
          console.log(`   💡 Interface: ${targetInterface} (index: ${interfaceIndex})`);
          console.log(`   💡 Gebruik dit commando om routing te configureren:`);
          console.log(`   💡 route add ${options.host} mask 255.255.255.255 ${localAddress} if ${interfaceIndex} -p`);
        }
      }
      
      // Probeer socket te binden met expliciete opties
      // Op Windows werkt dit mogelijk niet volledig vanwege netwerkstack beperkingen
      const socket = new net.Socket({
        allowHalfOpen: false
      });
      ...
      try {
        // Bind aan het specifieke IP adres met poort 0 (laat OS poort kiezen)
        socket.bind(0, localAddress, (bindErr) => {
          if (bindErr) {
            console.error(`   ❌ Fout bij binden aan ${localAddress}: ${bindErr.message} (code: ${bindErr.code})`);
          ...
          if (boundAddr.address !== localAddress) {
            console.warn(`   ⚠️ Socket gebonden aan ${boundAddr.address} i.p.v. ${localAddress}`);
            console.warn(`   💡 Windows gebruikt het eerste IP adres van de interface`);
            console.warn(`   💡 Dit is een bekende Windows beperking`);
            
            if (isWindows && targetInterface && interfaceIndex) {
              console.warn(`   💡 OPLOSSING: Configureer Windows routing tabel:`);
              console.warn(`   💡 route add ${options.host} mask 255.255.255.255 ${localAddress} if ${interfaceIndex} -p`);
              console.warn(`   💡 Of gebruik ForceBindIP tool voor deze applicatie`);
            }
          }
          ...
          socket.connect(options.port, options.host, () => {
            ...
            if (localAddr !== localAddress) {
              ...
              if (isWindows && targetInterface && interfaceIndex) {
                console.warn(`   💡 OPLOSSING: Configureer Windows routing tabel:`);
                console.warn(`   💡 route add ${options.host} mask 255.255.255.255 ${localAddress} if ${interfaceIndex} -p`);
                console.warn(`   💡 Of gebruik ForceBindIP (https://r1ch.net/projects/forcebindip) om deze applicatie te forceren`);
              }
            } else {
              console.log(`   ✅ Bevestigd: Socket gebruikt correct IP adres ${localAddress}`);
            }
