...
export async function createServer(cfg){
  ...
  // Bewaar serverIP voor gebruik in sendViaSMTP
  const serverIP = svc.serverIP || "0.0.0.0";
  ...
  const server=new SMTPServer({
            name: listenAddress !== "0.0.0.0" ? listenAddress : "rileesurfis",
  ...
  log("config.info", { message: `Starting SMTP server on port ${port}`, host: listenAddress });
  
  // Gebruik geconfigureerd IP adres of fallback naar alle interfaces
  server.listen(port, listenAddress, () => {
    directLog(`✅ SMTP server succesvol gestart op ${listenAddress}:${port}`);
    log("config.info", { message: `SMTP server ready on ${listenAddress}:${port}`, port: port, host: listenAddress });
