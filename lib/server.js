import { SMTPServer } from "smtp-server"; import fs from "fs"; import path from "path"; import { fileURLToPath } from "url"; import Matcher from "cidr-matcher"; import { simpleParser } from "mailparser"; import { v4 as uuidv4 } from "uuid"; import { sendViaGraph, clearMsalCache } from "./graph.js"; import { sendViaSMTP } from "./smtp.js"; import { checkRate } from "./rateLimiter.js";

// Standaard maximale berichtgrootte (bytes) als er geen service.maxMessageSizeKB is ingesteld.
// Dit is een harde bovengrens voor wat er in het geheugen gebufferd wordt; tenants kunnen
// via policy.maxMessageSizeKB een lagere limiet afdwingen.
...
// Accepteert zowel CIDR notatie (192.168.0.0/24) als losse IP-adressen (192.168.1.1)
// Losse IP-adressen worden automatisch geconverteerd naar /32 CIDR notatie
const cidr=(ip,list=[])=>{
  if(!ip || !list || list.length === 0) return false;
  
  // Filter en valideer de lijst: verwijder lege strings, null, undefined en ongeldige IP/CIDR notaties
  const validRanges = list
    ...
    .map(range => {
      ...
      // Als het een los IP-adres is (zonder /prefix), voeg /32 toe voor enkel IP-adres
      if (!range.includes('/')) {
        return `${range}/32`;
      }
    ...
    .filter(range => range !== null); // Verwijder ongeldige entries
...
export async function createServer(cfg){
  ...
  // Globale IP allowlist: wordt bij elke verbinding dynamisch uit `svc.allowlistIPs` gelezen
  // via de cidr() helper, zodat een hot-reload van config.json direct effect heeft.
  const haveTLS=!!(svc.tls?.certFile && svc.tls?.keyFile); const tlsMode=svc.tls?.mode||"starttls";
  const maxMessageBytes = () => (svc.maxMessageSizeKB ? svc.maxMessageSizeKB * 1024 : DEFAULT_MAX_MESSAGE_BYTES);
  ...
  async function reloadTenants() {
    try {
      ...
      // Gecachte MSAL apps weggooien: tenant credentials/certificaten kunnen gewijzigd zijn
      clearMsalCache();
  ...
  const server=new SMTPServer({
    ...
    // Adverteer SIZE zodat nette clients grote berichten niet eens proberen; de harde
    // limiet wordt daarnaast in onData afgedwongen (smtp-server zet alleen stream.sizeExceeded).
    ...
    onConnect(s,cb){
      ...
      if(!cidr(s.remoteAddress, svc.allowlistIPs)){
        const id = uuidv4();
    ...
    onData(st,s,cb){ const id=uuidv4(); const chunks=[]; let received=0; let sizeExceeded=false; const limit=maxMessageBytes();
      log("debug", {
        msgId: id,
      ...
      // Buffer chunks in een array (één Buffer.concat aan het eind i.p.v. O(n²) hergroeien)
      // en stop met bufferen zodra de limiet overschreden is: de rest van de stream wordt
      // weggegooid zodat een client het geheugen niet kan laten vollopen.
      st.on("data",ch=>{
        received+=ch.length;
        if(sizeExceeded) return;
        if(received>limit){ sizeExceeded=true; chunks.length=0; return; }
        chunks.push(ch);
      });
      st.on("end",async()=>{
      const ip=s.remoteAddress; let tenantName="unknown"; let fromAddr=""; let rcptCount=0; let rcpts=[]; try{
        if(sizeExceeded || st.sizeExceeded) {
          throw new Error(`Message too large (${Math.ceil(received/1024)}KB > ${Math.ceil(limit/1024)}KB)`);
        }
        ...
        // Let op: de specifiekere "IP not allowed for tenant" moet vóór de algemene "IP not allowed" staan
  ...
  // Eén fallback-poging op localhost; daarna het proces beëindigen zodat de service
  // manager (NSSM) een herstart kan doen. Zonder deze guard roept een mislukte
  // fallback dezelfde 'error' handler opnieuw aan en ontstaat een oneindige retry-loop.
  ...
  server.on('error', (err) => {
    ...
    if (!listenRetried && (err.code === 'EADDRINUSE' || err.code === 'EACCES')) {
      listenRetried = true;
      log("config.warn", { message: `Retrying bind on localhost:${port}` });
      server.listen(port, "127.0.0.1", () => {
        log("config.info", { message: `SMTP server ready on localhost:${port} (fallback)` });
      });
      return;
    }

    originalConsoleError(`❌ SMTP server kan niet luisteren op poort ${port}; proces wordt beëindigd`);
    log("config.error", { message: `SMTP server could not bind on port ${port}, exiting`, code: err.code });
    process.exit(1);
  });
