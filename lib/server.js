...
export async function createServer(cfg){
  let tenants=cfg.tenants; let svc=cfg.service; const smtpServers=svc.smtpServers||[];
  ...
  async function reloadTenants() {
    try {
      ...
      // Update service configuratie (inclusief routingPriority)
      if (config.service) {
        svc = config.service;
        log("config.info", { 
          message: "Service configuration reloaded (including routingPriority)", 
  ...
  const server=new SMTPServer({
    ...
    onConnect(s,cb){
      ...
      const ipInAnyTenant = tenants.some(t => 
        t.routing?.ipRanges && t.routing.ipRanges.length > 0 && cidr(s.remoteAddress, t.routing.ipRanges)
      );
