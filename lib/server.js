import { SMTPServer } from "smtp-server"; import fs from "fs"; import path from "path"; import Matcher from "cidr-matcher"; import { simpleParser } from "mailparser"; import { v4 as uuidv4 } from "uuid"; import { sendViaGraph } from "./graph.js"; import { sendViaSMTP } from "./smtp.js"; import { checkRate } from "./rateLimiter.js";
...
export async function createServer(cfg){
  ...
  async function reloadTenants() {
    try {
      ...
      // Update service configuratie (inclusief routingPriority en smtpServers)
      if (config.service) {
        svc = config.service;
        log("config.info", { 
          message: "Service configuration reloaded (including routingPriority and smtpServers)", 
  ...
  const server=new SMTPServer({
    ...
    host: "0.0.0.0",
    onAuth(a,s,cb){ if(!svc.optionalBasicAuth){const ok=(svc.authUsers||[]).some(u=>u.username===a?.username && u.password===a?.password); return ok?cb(null,{user:a.username}):cb(new Error("Invalid credentials"));} else { return cb(null,{user:a?.username||"anonymous"});} },
    ...
    onData(st,s,cb){ const id=uuidv4(); let raw=Buffer.alloc(0); 
      ...
      const ip=s.remoteAddress; let tenantName="unknown"; let fromAddr=""; let rcptCount=0; let rcpts=[]; try{
        ...
        if (deliveryMethod === "smtp") {
          ...
          try {
            await sendViaSMTP({tenant:t, parsed, rcpts, envelopeFrom:from, smtpServer});
            log("deliver.ok",{msgId:id,tenant:t.name,from,rcptCount:rcpts.length,rcpts:rcpts.join(", "),sizeKB,remoteIP:ip,deliveryMethod:"smtp",smtpServer:smtpServerName});
          } catch (smtpError) {
            log("deliver.err",{msgId:id,tenant:t.name,from,rcptCount:rcpts.length,rcpts:rcpts.join(", "),sizeKB,remoteIP:ip,deliveryMethod:"smtp",smtpServer:smtpServerName,error:smtpError.message,reason:"smtp_send_failed"});
            throw smtpError;
          }
        } else {
          ...
          log("deliver.ok",{msgId:id,tenant:t.name,from,rcptCount:rcpts.length,rcpts:rcpts.join(", "),sizeKB,remoteIP:ip,deliveryMethod:"graph"});
        }
  ...
  directLog(`🔌 SMTP server starten op poort ${port} (0.0.0.0:${port})`);
  log("config.info", { message: `Starting SMTP server on port ${port}`, host: "0.0.0.0" });
  
  // Forceer binding op alle interfaces, ook in service context
  server.listen(port, "0.0.0.0", () => {
    directLog(`✅ SMTP server succesvol gestart op 0.0.0.0:${port}`);
    log("config.info", { message: `SMTP server ready on 0.0.0.0:${port}`, port: port, host: "0.0.0.0" });
