...
/**
 * Download een bestand van een remote repository
 * @param {string} repoUrl - Base URL van de repository (bijv. https://raw.githubusercontent.com/owner/repo/branch)
 * @param {string} filePath - Relatief pad naar het bestand
 * @returns {Promise<Buffer>} Bestandsinhoud als Buffer
 */
export async function downloadFile(repoUrl, filePath) {
  try {
    // Normaliseer repoUrl (verwijder trailing slash)
    ...
    // Maak een AbortController voor timeout
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 seconden timeout voor downloads
    ...
    try {
      ...
      clearTimeout(timeoutId);
      if (fetchError.name === 'AbortError') {
        throw new Error(`Timeout: Kon geen verbinding maken met ${fileUrl} binnen 30 seconden`);
      }
      throw fetchError;
    }
  } catch (error) {
    console.error(`❌ Fout bij downloaden bestand ${filePath}:`, error.message);
    throw error;
  }
}

/**
 * Controleer of een bestand moet worden uitgesloten van updates
 * @param {string} filePath - Relatief pad naar het bestand
 * @returns {boolean} True als bestand moet worden uitgesloten
 */
...
/**
 * Maak een backup van een bestand voordat het wordt overschreven
 * @param {string} filePath - Volledig pad naar het bestand
 * @returns {string|null} Pad naar backup bestand of null als backup niet mogelijk is
 */
export function backupFile(filePath) {
  try {
    if (!fs.existsSync(filePath)) {
      return null; // Geen backup nodig als bestand niet bestaat
    }
    ...
    console.error(`⚠️ Kon geen backup maken van ${filePath}:`, error.message);
    return null;
  }
}

/**
 * Valideer gedownloade bestandsinhoud tegen verwachte hash
 * @param {Buffer} content - Bestandsinhoud
 * @param {string} expectedHash - Verwacht SHA256 hash
 * @returns {boolean} True als hash overeenkomt
 */
export function validateFileHash(content, expectedHash) {
